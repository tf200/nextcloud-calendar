...
class GoogleController extends Controller {
	private const SESSION_STATE_KEY = 'calendar_google_oauth_state';
	...
	public function callback(?string $code = null, ?string $state = null, ?string $error = null): RedirectResponse {
		...
		try {
			...
			$this->logger->warning('Google Calendar OAuth connection failed: ' . $e->getMessage(), [
				'app' => $this->appName,
				'exception' => $e,
			]);
	...
	public function disconnect(): JsonResponse {
		...
		try {
			...
			$this->logger->warning('Google Calendar OAuth disconnect failed: ' . $e->getMessage(), [
				'app' => $this->appName,
				'exception' => $e,
			]);

			return JsonResponse::error('Could not disconnect Google account.');
		}
