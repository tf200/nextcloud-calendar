...
use OCA\Calendar\Db\GoogleSyncMap;
...
class GoogleSyncService {
	...
	public function ensureSyncCalendar(GoogleAuth $auth): string {
		$accessToken = $this->tokenService->getAccessToken($auth);
		$calendarId = $this->apiService->getOrCreateSyncCalendar($accessToken, $auth->getGoogleCalendarId());
		if ($auth->getGoogleCalendarId() !== $calendarId) {
			$auth->setGoogleCalendarId($calendarId);
	...
	public function syncInitial(string $userId): void {
		...
		foreach ($calendar->search('', [], ['timerange' => ['start' => $start, 'end' => $end], 'types' => ['VEVENT']], 1000) as $event) {
			try {
				...
				$this->logger->warning('Initial Google Calendar sync failed for one event: ' . $e->getMessage(), ['app' => 'calendar', 'exception' => $e]);
			}
		}
	...
	public function deleteCalendarObject(string $userId, array $calendarData, array $objectData): void {
		...
		$auth = $this->authMapper->findByUserId($userId);
		$accessToken = $this->tokenService->getAccessToken($auth);
		$this->apiService->deleteEvent($accessToken, $syncMap->getGoogleCalendarId(), $syncMap->getGoogleEventId());
	...
	public function retryFailed(): void {
		foreach ($this->syncMapMapper->findFailed() as $syncMap) {
			try {
				...
				$this->logger->warning('Google Calendar sync retry failed: ' . $e->getMessage(), ['app' => 'calendar', 'exception' => $e]);
			}
		}
	}

	private function upsertMappedEvent(GoogleAuth $auth, string $calendarUri, string $objectUri, array $mapped): void {
		$googleCalendarId = $this->ensureSyncCalendar($auth);
		$accessToken = $this->tokenService->getAccessToken($auth);
		...
		try {
			...
			$this->apiService->updateEvent($accessToken, $syncMap->getGoogleCalendarId(), $syncMap->getGoogleEventId(), $mapped['payload']);
		} catch (DoesNotExistException) {
			$syncMap = new GoogleSyncMap();
			...
			$syncMap->setGoogleCalendarId($googleCalendarId);
			$syncMap->setGoogleEventId($this->apiService->insertEvent($accessToken, $googleCalendarId, $mapped['payload']));
