...
class MicrosoftOAuthService {
	...
	public function exchangeCode(string $code): array {
		...
		if (empty($data['refresh_token']) || !is_string($data['refresh_token'])) {
			throw new RuntimeException('Microsoft did not return a refresh token. Revoke app access in Microsoft and connect again.');
		}
	...
	/**
	 * @return array{mail:string, id?:string, displayName?:string}
	 */
	public function fetchUserInfo(string $accessToken): array {
		...
		if ($response->getStatusCode() < 200 || $response->getStatusCode() >= 300) {
			throw new RuntimeException('Microsoft userinfo request failed.');
		}
		if (empty($data['mail']) || !is_string($data['mail'])) {
	...
	private function decodeJsonResponse(string $body, string $context): array {
		...
		if (!is_array($data)) {
			throw new RuntimeException(sprintf('Invalid Microsoft %s response.', $context));
		}
